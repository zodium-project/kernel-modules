...
say ""
say "${MAGENTA}${BOLD}╔══════════════════════════════════════════╗${NC}"
say "${MAGENTA}${BOLD}║   ◈  new-lg4ff kmod build               ║${NC}"
say "${MAGENTA}${BOLD}╚══════════════════════════════════════════╝${NC}"
say ""
...
info "Installing akmod-new-lg4ff..."
dnf install -y --setopt=install_weak_deps=False akmod-new-lg4ff
ok "akmod-new-lg4ff installed"

# ── Build kmod ────────────────────────────────────────────────
info "Building new-lg4ff kmod for ${KERNEL_VERSION}..."
akmods --force --kernels "${KERNEL_VERSION}" --kmod new-lg4ff
ok "new-lg4ff kmod built"
...
# ── Locate built RPM ──────────────────────────────────────────
info "Locating built new-lg4ff kmod RPM..."
shopt -s nullglob
RPMS=(/var/cache/akmods/new-lg4ff/kmod-new-lg4ff-*.rpm)
shopt -u nullglob

[[ ${#RPMS[@]} -gt 0 ]] || \
    (cat /var/cache/akmods/new-lg4ff/*.failed.log 2>/dev/null; \
     fail "No new-lg4ff kmod RPM found in /var/cache/akmods/new-lg4ff/")
...
say ""
say "${MAGENTA}${BOLD}╔══════════════════════════════════════════════════╗${NC}"
say "${MAGENTA}${BOLD}║   ◆  new-lg4ff kmod build complete               ║${NC}"
say "${MAGENTA}${BOLD}╚══════════════════════════════════════════════════╝${NC}"
say ""