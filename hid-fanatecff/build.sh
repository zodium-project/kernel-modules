#!/usr/bin/env bash
# ================================================================
#  hid-fanatecff — kmod build script
# ================================================================
...
say ""
say "${MAGENTA}${BOLD}╔══════════════════════════════════════════╗${NC}"
say "${MAGENTA}${BOLD}║   ◈  hid-fanatecff kmod build           ║${NC}"
say "${MAGENTA}${BOLD}╚══════════════════════════════════════════╝${NC}"
say ""
...
info "Installing akmod-hid-fanatecff..."
dnf install -y --setopt=install_weak_deps=False akmod-hid-fanatecff
ok "akmod-hid-fanatecff installed"

# ── Build kmod ────────────────────────────────────────────────
info "Building hid-fanatecff kmod for ${KERNEL_VERSION}..."
akmods --force --kernels "${KERNEL_VERSION}" --kmod hid-fanatecff
ok "hid-fanatecff kmod built"
...
# ── Locate built RPM ──────────────────────────────────────────
info "Locating built hid-fanatecff kmod RPM..."
shopt -s nullglob
RPMS=(/var/cache/akmods/hid-fanatecff/kmod-hid-fanatecff-*.rpm)
shopt -u nullglob

[[ ${#RPMS[@]} -gt 0 ]] || \
    (cat /var/cache/akmods/hid-fanatecff/*.failed.log 2>/dev/null; \
     fail "No hid-fanatecff kmod RPM found in /var/cache/akmods/hid-fanatecff/")
...
say ""
say "${MAGENTA}${BOLD}╔══════════════════════════════════════════════════╗${NC}"
say "${MAGENTA}${BOLD}║   ◆  hid-fanatecff kmod build complete          ║${NC}"
say "${MAGENTA}${BOLD}╚══════════════════════════════════════════════════╝${NC}"
say ""